...
void RTC_Init(void) {
	...
	RTC_Set_Calendar_Date(RTC_WEEKDAY_TUESDAY, 12, RTC_MONTH_MARCH, 21); /* [TODO] These values are stubs - fill in current date */
	
	// Configure the Time 
	RTC_Set_Time(RTC_TR_PM, 7, 30, 0); /* [TODO] These values are stubs - fill in current time */
...
void RTC_Set_Calendar_Date(uint32_t WeekDay, uint32_t Day, uint32_t Month, uint32_t Year) {
	RTC->DR = (__RTC_CONVERT_BIN2BCD(WeekDay) << RTC_POSITION_DR_WDU)
			| (__RTC_CONVERT_BIN2BCD(Day) << RTC_POSITION_DR_DU)
			| (__RTC_CONVERT_BIN2BCD(Month) << RTC_POSITION_DR_MU)
			| (__RTC_CONVERT_BIN2BCD(Year) << RTC_POSITION_DR_YU);
}

void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second) {
	RTC->TR = Format12_24
			| (__RTC_CONVERT_BIN2BCD(Hour) << RTC_POSITION_TR_HU)
			| (__RTC_CONVERT_BIN2BCD(Minute) << RTC_POSITION_TR_MU)
			| __RTC_CONVERT_BIN2BCD(Second);
}
...
uint32_t RTC_TIME_GetSecond(void) {
	return RTC->TR & (RTC_TR_ST | RTC_TR_SU);
}
